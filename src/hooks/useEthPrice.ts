...
export const useEthPrice = () => {
  ...
  const fetchEthPrice = async () => {
    try {
      ...
      const response = await axios.get<PriceData>(`${COINGECKO_API}/simple/price?ids=ethereum&vs_currencies=usd`, {
        timeout: 5000,
        ...
        cacheRef.current = {
          data: {
            ethereum: {
              ...
              usdc: response.data.ethereum.usd, // USDC is typically pegged to USD
            }
          },
          timestamp: Date.now(),
        };
